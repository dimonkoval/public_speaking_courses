...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query("SELECT DISTINCT c FROM User u JOIN u.courses c JOIN FETCH c.skills s JOIN FETCH c.modules m "
            + "JOIN FETCH m.themas t JOIN FETCH t.lessons l JOIN FETCH l.homeworks WHERE u.id = :userId")
    Set<Course> getCoursesByStudentId(@Param("userId") Long userId);
    ...
    @Query("SELECT DISTINCT s FROM User u JOIN u.courses c JOIN  c.skills s WHERE u.id = :userId")
    Set<Skill> getSkillsByUserId(@Param("userId") Long userId);

    @Query("SELECT DISTINCT h FROM User u JOIN u.courses c JOIN c.skills s JOIN c.modules m JOIN m.themas t "
            + "JOIN t.lessons l JOIN l.homeworks h WHERE u.id = :userId")
    Set<Homework> getHomeworksByUserId(@Param("userId") Long userId);

    @Query("SELECT u FROM User u WHERE u.id = :userId")
    Optional<User> getUserById(@Param("userId") Long userId);

    @Query("SELECT u FROM User u JOIN FETCH u.courses c JOIN  FETCH c.skills s JOIN  FETCH c.modules m "
            + "JOIN FETCH m.themas t JOIN FETCH t.lessons l JOIN FETCH l.homeworks WHERE u.id = :userId")
